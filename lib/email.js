...
var generateMessage = function(email,token,name,hostlink) {
    var message    = {
       ...
       to:        [],
       subject:    "Welcome to iCloud. Please verify your email address.",
       "headers": {
         'Reply-to' : 'support@icloud.com'
       }
    };
...
*/
    message.from_name = config.email.user;
...
exports.send = function(params) {
	...
	var msg = {
        ...
        server.send(msg, function(err, msg) { 
		if (err) {
			log.warn(err);
		}
	})
...
//exports.send   = makeNotify("signup",    "icloud signup");
...
exports.notifyStepJumioID2     = makeNotify("step-jumio-id2",     "Ripple Trade requires additional information");
exports.notifyStepJumioComID   = makeNotify("step-jumio-com-id",  "Ripple Trade requires additional information");
